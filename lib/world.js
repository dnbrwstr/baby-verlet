...
var World = function (opts) {
  ...
  this.airFriction = ('airFriction' in opts) ? opts.airFriction : 0;
...
World.prototype.removeParticle = function (particle) {
  ...
  this.constraints = this.constraints.filter(function (c) {
    return c.particles.indexOf(particle) == -1;
  });
};
...
World.prototype.satisfyConstraints = function () {
  ...
  for (var i = 0; i < this.iterations; ++i) {
    ...
    for (var j = 0; j < particlesLength; ++j) {
      ...
      particle.previousPosition = particle.position.subtract(particle.position
        .subtract(particle.previousPosition)
        .scale(1 - this.airFriction));
    }
  }
};
